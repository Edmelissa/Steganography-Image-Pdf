...
namespace КР
{
    class ImageSteg
    {
        ...
        {            
            ...
            salt = 8 + (((2 * 3 * image.Width * image.Height) / 8) % 8);
        ...
        public long SizeImage()
        {
            ...
            if (depth == "Format32bppArgb" || depth == "Format24bppRgb")
                size = size * 3; // кол-во пикселей умноженное на кол-во байт под RGB
        ...
        public Bitmap TextToImage(byte[] bytestext)
        {
            ...
            if(depth == "Format24bppRgb" || depth == "Format32bppArgb")
            {
        ...
        {
            BitArray bitstext = new BitArray(Convert.ToInt32(SizeImage()*2));

            Console.WriteLine("Распаковка");

            byte[] text = new byte[bitstext.Length/8 + 8];
